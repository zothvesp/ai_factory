...
class MsSqlServerDiff(MsSqlServer):
    ...
    def create_backup(self, params):
        ...
        sql = 'BACKUP DATABASE [{0}] TO DISK = "{1}" WITH {2} {3} {4} DIFFERENTIAL, DESCRIPTION = "{5}", NAME = "{6}";'
        ...
        if "BACKUP DATABASE is terminating abnormally" in res:
            if "BACKUP DATABASE WITH DIFFERENTIAL successfully processed" not in res:
    ...
    def restore(self, restore_data):
        try:
            ...
            if sql_settings is not None and sql_settings["VerifyOnly"]:
                ...
                cmd = 'RESTORE DATABASE [{0}] FROM DISK = "{1}" WITH {2};'.format(new_db_name, restore_data["path_to_backup"], recovery_mode)
                res = try_with_full_access_folder((lambda: self.make_tcp_request(" -Q '{0}'".format(cmd))))
                if "State" in res:
                    raise Exception(self.cm["FAILED_DIFF_RESTORE"].format(new_db_name, res))
...
# okay decompiling /home/lm/PycharmProjects/backs/pyc/sqlbak/dbms/mssql/diff.pyc
